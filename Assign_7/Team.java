 ...
 * It get and set values for each team, update scores and totalPoints
 * Also writes updated data to file
*/
public class Team {
  ...
  // @param gF int increment total number of goals for
  private void setGoalsFor(int gF){
    this.goalsFor += gF;
  }
  
  //@param gA int increment total number of goals against
  private void setGoalsAgainst(int gA){
    this.goalsAgainst += gA;
  }
  
  //@param delta int increment total number of points accordingly if (WIN, TIE, LOSS)
  ...
  public void play (int gF, int gA){
    if (gF == -1 || gA == -1){ // team did not play: -1 as null
    } else { 
      setGoalsFor(gF);
      setGoalsAgainst(gA);
